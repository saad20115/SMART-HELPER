  ...
  async restoreBackup(filename: string) {
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      if (data.classifications?.length)
        await tx.classification.createMany({ data: data.classifications });

      if (data.employees?.length)
        await tx.employee.createMany({ data: data.employees });

      if (data.leaveBalances?.length)
        await tx.leaveBalance.createMany({ data: data.leaveBalances });
      if (data.leaveTransactions?.length)
        await tx.leaveTransaction.createMany({ data: data.leaveTransactions });
      ...
      if (data.importLogs?.length)
        await tx.importLog.createMany({ data: data.importLogs }); // Has to be before errors
      if (data.importErrors?.length)
        await tx.importError.createMany({ data: data.importErrors });
    });
