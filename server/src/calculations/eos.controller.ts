...
export class EosController {
    ...
    async calculateEOS(@Param('employeeId') employeeId: string) {
        return this.eosService.calculateEOS(employeeId);
    }
    ...
    async getAggregatedCalculations(@Query() query: AggregatedCalculationsQueryDto) {
