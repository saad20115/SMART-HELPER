...
export class EosCalculationService {
     ...
     * @param employeeId
     */
    async calculateEOS(employeeId: string) {
        ...
        const { hireDate, endDate, basicSalary, housingAllowance, transportAllowance, otherAllowances, terminationType } = employee;
        ...
        if (leaveBalance && Number(leaveBalance.calculatedRemainingDays) > 0) {
            ...
            leaveCompensation = dailyWage * Number(leaveBalance.calculatedRemainingDays);
        }
     ...
     */
    async calculateAggregatedEntitlements(query: AggregatedCalculationsQueryDto): Promise<AggregatedCalculationsResponseDto> {
        ...
        const classificationMap = new Map<string, { count: number; total: number }>();

        const calculationDate = query.fiscalYearEnd ? new Date(query.fiscalYearEnd) : new Date();
        ...
        for (const employee of employees) {
            ...
            const end = employee.endDate ? new Date(employee.endDate) : calculationDate;
            const durationInMs = end.getTime() - start.getTime();
            const durationInDays = durationInMs / (1000 * 60 * 60 * 24);
            const serviceYears = durationInDays / 365.25;
            ...
            let entitlementRatio = 1.0;
            if (employee.terminationType === 'RESIGNATION') {
                if (serviceYears < 2) {
                    entitlementRatio = 0;
                } else if (serviceYears < 5) {
            ...
            const leaveBalanceDays = Math.max(0, accruedDays + manualAdjustments - manualUses);
            ...
            if (!branchMap.has(branchKey)) {
                branchMap.set(branchKey, { count: 0, total: 0 });
            }
            ...
            if (!jobTitleMap.has(jobTitleKey)) {
                jobTitleMap.set(jobTitleKey, { count: 0, total: 0 });
            }
            ...
            if (!departmentMap.has(departmentKey)) {
                departmentMap.set(departmentKey, { count: 0, total: 0 });
            }
            ...
            if (!classificationMap.has(classificationKey)) {
                classificationMap.set(classificationKey, { count: 0, total: 0 });
            }
        ...
        return {
            summary: {
                ...
                averageServiceYears: employees.length > 0 ? Number((totalServiceYears / employees.length).toFixed(2)) : 0
            },
            employees: employeeEntitlements,
            branchBreakdown: Array.from(branchMap.entries()).map(([branch, data]) => ({
                branch,
                employeeCount: data.count,
                totalEntitlements: Number(data.total.toFixed(2))
            })),
            jobTitleBreakdown: Array.from(jobTitleMap.entries()).map(([jobTitle, data]) => ({
                jobTitle,
                employeeCount: data.count,
                totalEntitlements: Number(data.total.toFixed(2))
            })),
            departmentBreakdown: Array.from(departmentMap.entries()).map(([department, data]) => ({
                department,
                employeeCount: data.count,
                totalEntitlements: Number(data.total.toFixed(2))
            })),
            classificationBreakdown: Array.from(classificationMap.entries()).map(([classification, data]) => ({
                classification,
                employeeCount: data.count,
                totalEntitlements: Number(data.total.toFixed(2))
            })),
